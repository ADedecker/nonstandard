...
do  `(filter.germ.lift_pred %%p %%x ↔ %%rhs) ← (return tgt) | fail "Goal is not an equivalence (step 3)",
    match p with
    | `(λ _, %%q) := 
      do  (i, r) ← open_lambdas q,
          local_context >>= trace,
          trace i,
          trace r,
          match r with
          | `(∀ _ : %%tp, %%s) := 
            do trace tp, t' ← infer_type tp,
            (do t' ← infer_type tp,
                trace t',
                unify t' `(Prop),
                e ← to_expr ``(filter.germ.lift_pred_imp_iff_imp_lift_pred),
                trace e,
                skip) <|>
          ...
          | `(∃ _ : %%tp, %%s) := 
            (do unify tp `(Prop), 
                e ← to_expr ``(filter.germ.lift_pred_exists_prop_iff_and_lift_pred'),
...
example (l : ℝ) (u : ℕ → ℝ) :
  ...
  (∀ ε > 0, ∃ N : (hyperfilter ℕ : filter ℕ).germ ℕ, ∀ n ≥ N, germ.map abs (germ.map u n - ↑l) < ε) :=
begin
  transfer_step_one,
  transfer_step_two,
  transfer_step_three,
